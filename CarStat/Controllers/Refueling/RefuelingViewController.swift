        ...
        collectionView.rx.itemSelected
            .subscribe(onNext: { [weak self] indexPath in
                ...
                case .refueling:
                    ...
                    vc.viewModel.newDate.accept(last.date)
                    vc.viewModel.newFuelPrice.accept(last.refueling?.price)
                    vc.viewModel.newLiters.accept(last.refueling?.quantity)
                    vc.viewModel.newTotaalPrice.accept(last.refueling?.totalPrice)
...
//        let addButton: UIBarButtonItem = UIBarButtonItem(title: "+", style: .plain, target: self, action: #selector(addMileage))
        let add = UIBarButtonItem(image: UIImage(systemName: "plus"), style: .plain, target: self, action: #selector(addMileage))
